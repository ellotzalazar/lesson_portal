            ...
            <div class="card">
                ...
                <table id="example2" class="table table-condensed">
                    ...
                    <?php foreach ($yearlevels as $yearlevel):?>
                        ...
                        <tr>
                            <td><?= $this->Number->format($yearlevel->id) ?></td>
                            <td><?= h($yearlevel->name) ?></td>
                            <?php if($this->request->session()->read('Auth.User.role') == 1) { ?>
                                <td>
                                    <?= $this->Html->link(__('<i class="material-icons">mode_edit</i>'), ['action' => 'edit', $yearlevel->id], ['escape' => false]) ?>
                                </td>
                            <?php } ?>
                        </tr>
